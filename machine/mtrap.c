...
static unsigned int req = 255;
static unsigned int stop_hartid = 0;
static unsigned int resume_hartid = 0;
static unsigned int enclave_hartid = 0;
static unsigned int sm_cnt;
static int64_t timer_cnt[5] = {0};
static int64_t whole_timer_cnt = 0;
static int is_resume = 0;
static int64_t prev_arg0;
#define CYCLES 50
...
send_ipi:
    ...
    case SBI_SET_TIMER:
      ...
      timer_cnt[hartid]++;
      if(enclave_hartid == hartid) {
          printm("%d ", arg0);
          if(!(timer_cnt[hartid]%CYCLES)) {
            printm("[set timer: %d] hartid: %d, cnt: %lld arg0: %d  ", whole_timer_cnt, enclave_hartid, timer_cnt[enclave_hartid], arg0);
            printm("cpu: %d %d %d %d\r\n",
                cpu_get_is_enclave(1),
                cpu_get_is_enclave(2),
                cpu_get_is_enclave(3),
                cpu_get_is_enclave(4));
          }
        prev_arg0 = arg0;
      } 
#if __riscv_xlen == 32
    ...
    case SBI_SM_RUN_ENCLAVE:
      resume_hartid = read_csr(mhartid);
      enclave_hartid = resume_hartid;
      printm("[run] hartid: %d\r\n", resume_hartid);
      retval = mcall_sm_run_enclave(regs, arg0);
      break;
    case SBI_SM_EXIT_ENCLAVE:
      retval = mcall_sm_exit_enclave(regs, arg0);
      printm("[exit] cur: %d enclave: %d -> timer_cnt: %lld\r\n", read_csr(mhartid), enclave_hartid, timer_cnt[enclave_hartid]);
      enclave_hartid = 0;
      break;
    case SBI_SM_STOP_ENCLAVE:
      req = arg0;
      sm_cnt++;
      retval = mcall_sm_stop_enclave(regs, arg0, &stop_hartid);
      if(!(sm_cnt%CYCLES)) {
          printm("[stop_enclave] sm_cnt: %lld :hartid: %d req: %d\r\n", sm_cnt, stop_hartid, req);
          printm("[stop timer_cnt: %lld] %lld %lld %lld %lld\r\n",whole_timer_cnt,  timer_cnt[1], timer_cnt[2], timer_cnt[3], timer_cnt[4]);
      }
      if(retval != ENCLAVE_INTERRUPTED && retval != ENCLAVE_EDGE_CALL_HOST) {
          printm("[stop] fail: ret:%d req: %d hartid: %d\r\n", retval, req, stop_hartid);
      }
      if(resume_hartid != stop_hartid) {
          printm("[stop: retval: %d] hartid is changed: #%d->#%d (request: %d)\r\n", retval, resume_hartid, stop_hartid, req);
          printm("cpu: %d %d %d %d\r\n",
            cpu_get_is_enclave(1),
            cpu_get_is_enclave(2),
            cpu_get_is_enclave(3),
    ...
    case SBI_SM_RESUME_ENCLAVE:
      resume_hartid = read_csr(mhartid);
      if(resume_hartid != stop_hartid) {
          printm("[resume] hartid is changed: #%d->#%d (request: %d)\r\n", stop_hartid, resume_hartid, req);
          printm("cpu: %d %d %d %d\r\n",
            cpu_get_is_enclave(1),
            cpu_get_is_enclave(2),
            cpu_get_is_enclave(3),
      ...
      if(!(sm_cnt%CYCLES)) {
          printm("[%lld] resume_enclave sm_cnt: %lld :hartid: %d req: %d\r\n", timer_cnt[resume_hartid], sm_cnt, resume_hartid, req);
          printm("[resume timer_cnt: %lld] %lld %lld %lld %lld\r\n",whole_timer_cnt,  timer_cnt[1], timer_cnt[2], timer_cnt[3], timer_cnt[4]);
      }
      retval = mcall_sm_resume_enclave(regs, arg0, req);
      if(retval != ENCLAVE_SUCCESS) {
          printm("[resume] fail: ret:%d req: %d hartid: %d\r\n", retval, req, resume_hartid);
      }
      break;
