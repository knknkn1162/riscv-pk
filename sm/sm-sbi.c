...
uintptr_t mcall_sm_resume_enclave(uintptr_t* host_regs, unsigned long eid, unsigned long req)
{
  enclave_ret_code ret;
  ...
  ret = resume_enclave(host_regs, (unsigned int) eid, req);
  return ret;
}
...
uintptr_t mcall_sm_stop_enclave(uintptr_t* encl_regs, unsigned long request, unsigned int *hartid)
{
  enclave_ret_code ret;
  /* only an enclave itself can call this SBI */
  if (!cpu_is_enclave_context2(hartid)) {
    return ENCLAVE_SBI_PROHIBITED;
  }
